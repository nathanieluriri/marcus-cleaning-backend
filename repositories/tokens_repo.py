...
async def _resolve_access_token_id(accessToken: str, allow_expired: bool) -> str | None:
    try:
        ...
        decoded = (
            await decode_jwt_token_without_expiration(token=accessToken)
            if allow_expired
            else await decode_jwt_token(token=accessToken)
        )
        if not isinstance(decoded, dict):
            return None
        embedded_access_token = str(decoded.get("accessToken") or decoded.get("access_token") or "").strip()
        if not embedded_access_token:
            return None
        try:
            ObjectId(embedded_access_token)
        except errors.InvalidId:
            return None
        return embedded_access_token
...
async def delete_current_session_with_access_token_id(
    *,
    user_id: str,
    access_token_id: str,
) -> tuple[int, int]:
    try:
        access_object_id = ObjectId(access_token_id)
        access_filter: dict[str, object] = {"_id": access_object_id, "userId": user_id}
    except errors.InvalidId:
        access_filter = {"_id": access_token_id, "userId": user_id}
