...
from schemas.saved_address import CustomerSavedAddressCreateRequest, SavedAddressOut, SavedAddressPatchRequest
...
async def test_create_my_saved_address_sets_first_as_default(monkeypatch: pytest.MonkeyPatch):
    ...
    async def _stub_get_place_details(*, place_id: str):
        assert place_id == "pid-1"
        return _place()

    async def _stub_create_saved_address(payload):
        assert payload.isDefault is True
        assert payload.place.place_id == "pid-1"
    ...
    monkeypatch.setattr(saved_address_service, "get_place_details", _stub_get_place_details)
...
@pytest.mark.asyncio
async def test_update_my_saved_address_resolves_place_id(monkeypatch: pytest.MonkeyPatch):
    async def _stub_get_place_details(*, place_id: str):
        assert place_id == "pid-2"
        return PlaceOut(
            place_id="pid-2",
            name="Marina",
    ...
    async def _stub_update_saved_address_for_user(*, address_id: str, user_id: str, payload):
        assert address_id == "addr-1"
        assert user_id == "customer-123"
        assert payload.label == "Work"
        assert payload.place is not None
        assert payload.place.place_id == "pid-2"
        return _saved_address(address_id=address_id, user_id=user_id, label="Work")

    monkeypatch.setattr(saved_address_service, "get_place_details", _stub_get_place_details)
    monkeypatch.setattr(saved_address_service, "update_saved_address_for_user", _stub_update_saved_address_for_user)

    result = await saved_address_service.update_my_saved_address(
        user_id="customer-123",
        address_id="addr-1",
        payload=SavedAddressPatchRequest(label="Work", place_id="pid-2"),
    )
