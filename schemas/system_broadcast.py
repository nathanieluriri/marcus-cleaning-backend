...
class SystemBroadcastBase(BaseModel):
    audience: str = Field(min_length=2, max_length=40)
    channel: str = Field(min_length=2, max_length=40)
    title: str = Field(min_length=2, max_length=150)
    message: str = Field(min_length=2, max_length=5000)
    schedule_epoch: int | None = Field(default=None, ge=0)
    status: str = Field(default="queued", min_length=2, max_length=20)


class SystemBroadcastCreate(SystemBroadcastBase):
    date_created: int = Field(default_factory=lambda: int(time.time()))
    last_updated: int = Field(default_factory=lambda: int(time.time()))


class SystemBroadcastUpdate(BaseModel):
...
class SystemBroadcastOut(SystemBroadcastBase):
