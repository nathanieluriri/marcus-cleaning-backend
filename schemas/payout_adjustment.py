...
class PayoutAdjustmentBase(BaseModel):
    cleaner_id: str = Field(min_length=2)
    amount_minor: int
    currency: str = Field(min_length=3, max_length=3)
    adjustment_type: str = Field(min_length=2, max_length=40)
    booking_id: str | None = None
    reason: str = Field(min_length=2, max_length=300)
    approved: bool = False


class PayoutAdjustmentCreate(PayoutAdjustmentBase):
    date_created: int = Field(default_factory=lambda: int(time.time()))
    last_updated: int = Field(default_factory=lambda: int(time.time()))


class PayoutAdjustmentUpdate(BaseModel):
...
class PayoutAdjustmentOut(PayoutAdjustmentBase):
